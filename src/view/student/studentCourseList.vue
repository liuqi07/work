  ...
  <div>
    ...
    <Modal v-model="detailModal" :width="600">
      <Form inline :label-width="80">
        <Row>
          <FormItem label="分类总级:">
            <span>level {{lookCommentData.thirdLevel}}</span>
          </FormItem>
          <FormItem label="课表等级:">
            <span>level {{lookCommentData.courseLevel}}</span>
          </FormItem>
          <FormItem label="教师评级:" v-show="lookCommentData.isComment && lookCommentData.teacherCommentLevel">
            <span>level {{lookCommentData.teacherCommentLevel}}</span>
          </FormItem>
          <FormItem label="评级:" v-show="!lookCommentData.isComment">
            <i-select v-model="lookCommentData.teacherCommentLevel" style="width:200px">
              <i-option v-for="item in lookCommentData.thirdLevel" :value="item">level {{ item }}</i-option>
            </i-select>
          </FormItem>
        </Row>
        <Row>
          <FormItem label="教师评语:" v-show="!lookCommentData.isComment">
            <Checkbox-group v-model="checkGroup">
              <Checkbox :label="item.desc" v-for="(item) in lookCommentData.allLabels">
                <Tag :color="item.color">{{item.desc}}</Tag>
              </Checkbox>
            </Checkbox-group>
          </FormItem>
          <FormItem label="教师评语:" v-show="lookCommentData.isComment">
            <Tag class="comment-detail-title"
                 v-for="(item, index) in lookCommentData.labels" :color="item.color"
                 :key="index">{{item.desc}}
            </Tag>
          </FormItem>
        </Row>
        <Input v-model="lookCommentData.commentDesc" class="comment-detail-title" type="textarea"
               :value="lookCommentData.commentDesc" :rows="3"
               :readonly="lookCommentData.isComment" :disabled="lookCommentData.isComment"/>
      </Form>
      <div slot="footer">
        <Button type="primary" @click="commentLevelCommit" v-show="!lookCommentData.isComment">保存</Button>
        <Button type="primary" @click="detailModal=false" v-show="lookCommentData.isComment">关闭</Button>
      </div>
    </Modal>
  ...
  export default {
    data() {
      return {
        ...
        courseList: [],
        total: 0,
        lookCommentData: {labels: [], allLabels: [], teacherCommentLevel: ''},
        detailModal: false,
        lookTaskModal: false,
        lookExamData: {},
        lookExamModal: false,
    ...
    methods: {
      commentLevelCommit() {
        if (this.checkGroup.length === 0) {
          this.$Message.error('请勾选评语标签!')
        } else if (this.lookCommentData.commentDesc === null ||
          this.lookCommentData.commentDesc === undefined ||
          this.lookCommentData.commentDesc === ''
        ) {
          this.$Message.error('请填写评语!')
        } else {
          let labels = [];
          for (let check of this.checkGroup) {
            for (let label of this.lookCommentData.allLabels) {
              if (label.desc === check) {
                labels.push(label)
              }
            }
          ...
          params.append('tableId', this.lookCommentData.tableId);
          params.append('commentDesc', this.lookCommentData.commentDesc);
          params.append('level', this.lookCommentData.teacherCommentLevel);
          params.append('labelJsonStr', JSON.stringify(labels));
          http._post({
            vm: this,
